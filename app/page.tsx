    ...
    <>
      ...
      <main className="min-h-screen bg-gradient-to-b from-slate-900 via-slate-800 to-emerald-900 text-white">
        ...
        <section className="px-6 py-12 max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            { icon: '🗺️', title: '制覇マップ', desc: '都道府県・市区町村・スポットを記録。全国制覇を目指そう。' },
            { icon: '📷', title: '思い出記録', desc: '写真・日記で旅の思い出をいつまでも残せる。' },
            { icon: '🏆', title: 'ランキング＆バッジ', desc: '訪問数で全国ランキングに挑戦。バッジを集めよう！' },
          ].map((item) => (
        ...
        <section className="text-center py-10 px-6">
          <p className="text-slate-400 text-sm mb-6">制覇できるスポット数</p>
          <div className="inline-grid grid-cols-3 sm:grid-cols-5 gap-4 sm:gap-8 bg-white/5 border border-white/10 rounded-2xl px-8 py-6">
            {[
              { num: '47', label: '都道府県' },
              ...
              <div key={item.label} className="text-center">
                <div className="text-2xl sm:text-3xl font-bold text-emerald-400">{item.num}</div>
