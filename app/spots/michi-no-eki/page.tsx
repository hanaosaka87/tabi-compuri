...
const ALL_SPOTS: Spot[] = michiNoEkiData as Spot[]
...
export default function MichiNoEkiPage() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      setUserId(user.id)
      const { data } = await supabase.from('spot_visits').select('spot_id').eq('user_id', user.id).eq('category', 'michi-no-eki')
      if (data) setVisitedIds(new Set(data.map(v => v.spot_id)))
      setLoading(false)
    }
    init()
  }, [])
  ...
  const handleToggle = async (spot: Spot) => {
    ...
    if (visitedIds.has(spot.id)) {
      ...
      await supabase.from('spot_visits').insert({ user_id: userId, spot_id: spot.id, category: 'michi-no-eki', spot_name: spot.name, prefecture_code: spot.prefecture_code, visited_at: new Date().toISOString().split('T')[0] })
      setVisitedIds(prev => new Set(prev).add(spot.id))
    }
  }

  const filtered = ALL_SPOTS.filter(s => {
    const matchPref = selectedPref === 'all' || s.prefecture_code === selectedPref
    const matchSearch = search === '' || s.name.includes(search)
    return matchPref && matchSearch
  })

  const visitedInFiltered = filtered.filter(s => visitedIds.has(s.id)).length
    ...
    <main className="min-h-screen bg-slate-900 text-white">
      ...
      {isGuest && (
        <div className="bg-emerald-500/10 border-b border-emerald-500/30 px-4 py-3 flex items-center justify-between">
          ...
          <Link href="/login" className="bg-emerald-500 text-white px-4 py-1.5 rounded-full text-xs font-bold flex-shrink-0 ml-4">
            ログイン
          </Link>
        </div>
      )}
      ...
      <div className="max-w-3xl mx-auto px-4 py-6">
        ...
        <div className="mb-6">
          <div className="flex items-center gap-2 mb-1">
            <Link href="/spots" className="text-slate-500 text-sm hover:text-slate-300">スポット</Link>
            <span className="text-slate-600">›</span>
            <span className="text-sm text-white">道の駅</span>
          </div>
          <h1 className="text-2xl font-bold">🚗 道の駅制覇</h1>
        </div>

        {/* 制覇状況 */}
        <div className="bg-gradient-to-r from-emerald-900/50 to-slate-800/50 border border-emerald-500/20 rounded-2xl p-5 mb-6">
          <div className="flex items-center justify-between mb-3">
            <div>
              <p className="text-slate-400 text-xs">制覇済み</p>
              <p className="text-3xl font-bold">{totalVisited}<span className="text-slate-400 text-base font-normal"> / {ALL_SPOTS.length}</span></p>
            </div>
            <div className="text-right">
              <p className="text-emerald-400 text-2xl font-bold">{Math.round(totalVisited / ALL_SPOTS.length * 100)}%</p>
            </div>
          </div>
          <div className="w-full bg-slate-700 rounded-full h-2">
            <div className="bg-emerald-500 h-2 rounded-full transition-all" style={{ width: `${totalVisited / ALL_SPOTS.length * 100}%` }} />
          </div>
        </div>
        ...
        <p className="text-slate-400 text-xs mb-3">
          {filtered.length}件表示 {selectedPref !== 'all' || search ? `（${visitedInFiltered}件制覇済み）` : ''}
        </p>
