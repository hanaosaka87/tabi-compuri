...
function nearestPref(lat, lon) {
  ...
  for (const p of PREFS) {
    ...
    if (d < minDist) { minDist = d; nearest = p.code }
  }
  return nearest
}
...
`
  ...
  console.log('Wikidataからデータ取得中...')
  const res = await fetch(url, {
    headers: {
      'Accept': 'application/sparql-results+json',
      'User-Agent': 'tabi-compuri/1.0 (https://tabi-compuri.vercel.app)',
    },
  })
  ...
  const spots = rows
    ...
    .map((r, i) => {
      ...
      return {
        id: `michi-${id}`,
        name: r.name.value,
        prefecture_code: nearestPref(lat, lon),
        lat,
        lon,
      }
    })
    .sort((a, b) => a.prefecture_code - b.prefecture_code)

  return spots
}

async function main() {
  const spots = await fetchFromWikidata()
  mkdirSync('lib/spots', { recursive: true })
  writeFileSync('lib/spots/michi-no-eki.json', JSON.stringify(spots, null, 2), 'utf-8')
  console.log(`完了: ${spots.length}件 → lib/spots/michi-no-eki.json`)

  // 都道府県別の集計
  const byPref = {}
  for (const s of spots) {
    byPref[s.prefecture_code] = (byPref[s.prefecture_code] || 0) + 1
  }
  const prefNames = Object.fromEntries(PREFS.map(p => [p.code, p.name]))
  console.log('\n都道府県別件数:')
  for (const [code, count] of Object.entries(byPref).sort((a,b) => Number(a[0])-Number(b[0]))) {
    console.log(`  ${prefNames[code]}: ${count}件`)
  }
}
